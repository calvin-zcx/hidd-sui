...
if __name__ == '__main__':
    ...
    # Target data
    target_dataset = Dataset(target_data_1st_neg, diag_name=dx_name, diag_code_threshold=20)
    target_x, target_y, target_uid_list, target_y_more = target_dataset.flatten_to_tensor()
    # Source data for source model training
    ...
    target_x_transform = x_target_to_source(target_x, target_dataset.diag_code_vocab, x.shape[1], my_dataset.diag_code_vocab)
    ...
    if args.run_model in ['LR', 'XGBOOST', 'LIGHTGBM', 'SVM', 'KNN']:
        ...
        print('Applying source model directly to transformed target data: ')
        target_y_pre_prob = model.predict_proba(target_x_transform)
        target_y_transform = return_label(target_y)
        target_result_1 = ml.MLModels._performance_at_specificity_or_threshold(target_y_pre_prob, target_y_transform, specificity=0.9)
        threshold = result_2[1]
        target_y_pre_label_spec09 = (target_y_pre_prob > target_result_1[1]).astype(int)
        print('......Results at specificity 0.95:')
        target_result_2 = ml.MLModels._performance_at_specificity_or_threshold(target_y_pre_prob, target_y_transform, specificity=0.95)
        target_y_pre_label_spec095 = (target_y_pre_prob > target_result_2[1]).astype(int)
        target_auc = roc_auc_score(target_y_transform, target_y_pre_prob)
        pd.DataFrame(
            {'uid': target_uid_list,
             'y_label': target_y_transform,
             'y_pre_prob': target_y_pre_prob,
             'y_pre_label_at_spec09':target_y_pre_label_spec09,
             'y_pre_label_at_spec095': target_y_pre_label_spec095,
             }).to_csv(os.path.join(os.path.dirname(args.save_model_filename),
                                    'target_risk_by_source_{}r{}.csv'.format(args.run_model, args.random_seed)))
        target_df1 = pd.DataFrame([target_result_1 + (model.best_hyper_paras,), target_result_2 + (model.best_hyper_paras,),
                            ],
                           columns=["AUC", "threshold", "Specificity", "Sensitivity/recall", "PPV/precision",
                                    "n_negative", "n_positive", "precision_recall_fscore_support", 'best_hyper_paras'],
                           index=['r_9', 'r_95'])
        target_df1.to_csv(os.path.join(os.path.dirname(args.save_model_filename),
                                       'target_results_by_source_{}r{}.csv'.format(args.run_model, args.random_seed)))
