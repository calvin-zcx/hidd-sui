...
def pre_df_2_dict(df, outfile):
    ...
    for index, row in tqdm(df.iterrows(), total=len(df)):
        ...
        if (not pd.isna(col)) and (col != ''):
            try:
                dxs = [x.strip() for x in col.strip("\"\'").split(',') if (x.strip() != '')]
            except:
                print(index, myuid, col)
                raise ValueError
            # if len(dxs) > 1:
            #     print(dxs)
            for x in dxs:
...
def pre_dict_to_triples_1st_neg(uid_records, enfunc, encode, exclude1visit=False):
    ...
    if exclude1visit:
        foname = 'pickles/apcd/final_pats_1st_neg_triples_exclude1visit_apcd-{}.pkl'.format(encode)
    else:
        foname = 'pickles/apcd/final_pats_1st_neg_triples_apcd-{}.pkl'.format(encode)
...
def pre_dict_to_triples_1st_sui(uid_records, enfunc, encode):
    ...
    foname = 'pickles/apcd/final_pats_1st_sui_triples_apcd-{}.pkl'.format(encode)
    pickle.dump(triples, open(foname, 'wb'))
...
if __name__ == '__main__':
    ...
    with open(r'data/apcd/final_pats_1st_neg_apcd.pkl', 'rb') as f:
        df_1st_neg = pickle.load(f)
        data_1st_neg = pre_df_2_dict(df_1st_neg, 'pickles/apcd/final_pats_1st_neg_dict_apcd.pkl')
        ...
        data_1st_neg_triples = pre_dict_to_triples_1st_neg(data_1st_neg, enfunc, args.encode)  #, exclude1visit=True)

    with open(r'data/apcd/final_pats_1st_sui_apcd.pkl', 'rb') as f:
        df_1st_sui = pickle.load(f)
        data_1st_sui = pre_df_2_dict(df_1st_sui, 'pickles/apcd/final_pats_1st_sui_dict_apcd.pkl')
        print(len(data_1st_sui))
        data_1st_sui_triples = pre_dict_to_triples_1st_sui(data_1st_sui, enfunc, args.encode)

    a = []
    for k, v in data_1st_neg.items():
        a.append(len(v))
    n_of_visits = pd.DataFrame(a).value_counts()
    n_of_visits.to_csv('debug/no_of_visits_per_person_1st_neg_apcd.csv')
